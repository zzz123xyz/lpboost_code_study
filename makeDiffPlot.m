...
switch combnum
 ...
 case 29
 case 30, titlestr='SIFT - grey - K=300 (4 kernels)'; ax = [4.5 30.5 -4 2];
 case 31, 
 case 32, titlestr='PHOG: Angle-360,40 bins (4 kernels)'; ax = [4.5 30.5 -6 3];
 case 33, titlestr='Pyramid Level 2 kernels (8 kernels)'; ax = [4.5 30.5 -4 10];
 case 34, 
 case 39, 
  switch dataset
   case '101'
    titlestr=sprintf('Caltech-%s (39 kernels)',dataset); ax = [4.5 30.5 -10 11];
   case '256'
    titlestr=sprintf('Caltech-%s (39 kernels)',dataset); ax = [4.5 50.5 -4 15];
  end
...
switch dataset
 case '101'
  ...
  thisErrSingle = squeeze(err_single(nTrainPoints,:));
  meanAccSingle = mean(1-thisErrSingle,2);
  stdAccSingle = std((1-thisErrSingle)');
  errorbar(nTrainPoints,zeros(numel(nTrainPoints),1),100*stdAccSingle,color_separate,'LineWidth',2);
  %legend_text{1} = strrep(explain_feature(f{featnums(bestSingleMatrix)}),'_','\_');
  legend_text{1} = 'best feature';
  i=2;

  fprintf('Best Feature ');
  fprintf('&%.1f $\\pm$ %.1f',100*[meanAccSingle,stdAccSingle']');
  fprintf('\\\\\n');
  
  if (0)
      thisErr = squeeze(err_product(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
      meanDiff = mean(-thisErr,2);
      stdDiff = std(thisErr');
      errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_product,'LineWidth',2);
      legend_text{i} = 'product';
      i=i+1;
      fprintf('Product ');
      fprintf('&%.1f $\\pm$ %.1f ',100*[meanDiff,stdDiff']');
      fprintf('\\\\\n');
  end
  
  
  thisErr = squeeze(err_average(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
  meanDiff = mean(-thisErr,2);
  stdDiff = std(thisErr');
  errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_average,'LineWidth',2);
  ...
  if 0 & (combnum ~= 39)
      thisErr = squeeze(err_logistic(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
      meanDiff = mean(-thisErr,2);
      stdDiff = std(thisErr');
      errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_logistic,'LineWidth',2);
  ...
  thisErr = squeeze(err_silp(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
  meanDiff = mean(-thisErr,2);
  stdDiff = std(thisErr');
  errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_silp,'LineWidth',2);
  legend_text{i} = 'MKL';
  i=i+1;
  fprintf('MKL ');
  fprintf('&%.1f $\\pm$ %.1f ',100*[meanDiff,stdDiff']');
  ...
  thisErr = squeeze(err_lpbeta(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
  meanDiff = mean(-thisErr,2);
  stdDiff = std(thisErr');
  errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_lpbeta,'LineWidth',2);
  ...
  if (0)
  thisErr = squeeze(err_lpb(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
  meanDiff = mean(-thisErr,2);
  stdDiff = std(thisErr');
  errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_lpb,'LineWidth',2);
  legend_text{i} = 'LP-B';
  i=i+1;
  grid on
  ...
  thisErr = squeeze(err_lp_avg(combnum,nTrainPoints,:))-err_single(nTrainPoints,:);
  meanDiff = mean(-thisErr,2);
  stdDiff = std(thisErr');
  errorbar(nTrainPoints,100*meanDiff,100*stdDiff,color_lpa,'LineWidth',2);
  i=i+1;
  fprintf('LP-Avg ');
  fprintf('&%.1f $\\pm$ %.1f ',100*[meanDiff,stdDiff']');
  fprintf('\\\\\n');
...
ylabel('accuracy difference in %','FontSize',26);
